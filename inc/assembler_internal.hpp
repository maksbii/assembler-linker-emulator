...
public:
    ...
    // Emits a PC-relative load instruction (RegB=pc, RegC=0, Disp=0
    // placeholder) referencing a literal-pool slot that doesn't exist yet,
    // and queues that slot to be materialized by flushLiteralPool(). Used
    // for `ld` operands whose value doesn't fit the 12-bit Disp field
    // (oversized literal) or needs relocation (symbol address).
    void appendPoolLiteral(unsigned oc, unsigned mod, unsigned regA, long value) {
        long instr_addr = location_counter;
        appendInstruction(oc, mod, regA, REG_PC, 0, 0);
        pending_pool.push_back({instr_addr, false, value, ""});
    }

    void appendPoolSymbol(unsigned oc, unsigned mod, unsigned regA, const std::string &symbol) {
        long instr_addr = location_counter;
        appendInstruction(oc, mod, regA, REG_PC, 0, 0);
    ...
    // this one-pass assembler); RegC is always 0 for these instructions so
    // the backpatch can safely overwrite the whole low nibble of byte2.
    void flushLiteralPool(RelocationTable &relocation_table, int section_index) {
        for (const auto &entry : pending_pool) {
            ...
            memory[entry.instr_addr + 2] = (unsigned char)((d >> 8) & 0xF);
